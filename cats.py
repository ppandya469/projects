...
def pick(paragraphs, select, k):
    ...
    picker_list = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k >= len(picker_list):
        return ''
    else:
        return picker_list[k]
...
def about(topic):
    ...
    def picker(prarrgaph):
        i = remove_punctuation(prarrgaph).split()
        for x in topic:
            for h in i:
                if x.lower() == h.lower():
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for k in range(len(word_list)):
        if diff_function(typed_word, word_list[k], limit) < lowest:
            lowest = diff_function(typed_word, word_list[k], limit)
            var = k

    if diff_function(typed_word, word_list[var], limit) > limit:
        return typed_word
    return word_list[var]
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    prog = a / len(prompt)
    dicti = {'id': user_id, 'progress': prog}
    upload(dicti)
    return prog
...
def time_per_word(words, times_per_player):
    ...
    a = [[abs(times_per_player[b][i] - times_per_player[b][i+1]) for i in range(len(times_per_player[b])-1)] for b in range(len(times_per_player))]
...
def fastest_words(match):
    ...
    for k in word_indices:
        ...
        for j in player_indices:
            if time(match, j, k) < lowest:
                lowest = time(match, j, k)
                best_player = j
        final[best_player].append(get_word(match,k))
    return final
